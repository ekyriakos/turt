...
bool turtle_interfaces__msg__bounding_boxes__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("turtle_interfaces.msg._bounding_boxes.BoundingBoxes", full_classname_dest, 51) == 0);
  }
  turtle_interfaces__msg__BoundingBoxes * ros_message = _ros_message;
  {  // header
  ...
  {  // w
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      Py_ssize_t size = view.len / sizeof(double);
      if (!rosidl_runtime_c__double__Sequence__init(&(ros_message->w), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create double__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      double * dest = ros_message->w.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // color
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      Py_ssize_t size = view.len / sizeof(double);
      if (!rosidl_runtime_c__double__Sequence__init(&(ros_message->color), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create double__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      double * dest = ros_message->color.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // camera
    ...
    ros_message->camera = (uint8_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
PyObject * turtle_interfaces__msg__bounding_boxes__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("turtle_interfaces.msg._bounding_boxes");
  ...
  {  // w
    ...
    if (ros_message->w.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->w.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->w.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // color
    ...
    if (ros_message->color.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->color.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->color.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // camera
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->camera);
    {
