    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'turtle_interfaces.msg.BoundingBoxes')
            logger.debug(
...
class BoundingBoxes(metaclass=Metaclass_BoundingBoxes):
    ...
    def __init__(self, **kwargs):
        ...
        self.w = array.array('d', kwargs.get('w', []))
        self.h = array.array('d', kwargs.get('h', []))
        self.color = array.array('d', kwargs.get('color', []))
    ...
    def w(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'w' array.array() must have the type code of 'd'"
            self._w = value
            return
        if __debug__:
            from collections.abc import Sequence
    ...
    def color(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'color' array.array() must have the type code of 'd'"
            self._color = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'color' field must be a set or sequence and each value of type 'float'"
        self._color = array.array('d', value)
    ...
    def camera(self, value):
        if __debug__:
            ...
            assert value >= 0 and value < 256, \
                "The 'camera' field must be an unsigned integer in [0, 255]"
        self._camera = value
